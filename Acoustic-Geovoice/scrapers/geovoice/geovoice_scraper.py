...
async def scrape_geovoice_full():
    ...
    async with async_playwright() as p:
        ...
        for idx, product_url in enumerate(product_links, 1):
            ...
            # Random delay before each product page (15-25 seconds) - extra polite
            # because geovoice.ge shares the same server as acoustic.ge
            if idx > 1 and not no_delay:  # Skip delay for first product
                delay = random.uniform(15, 25)
                print(f"  Waiting {delay:.1f}s before visiting...", flush=True)
                await asyncio.sleep(delay)
            
            # Longer cooldown break every 40 products to stay under rate limits
            if idx > 1 and idx % 40 == 0 and not no_delay:
