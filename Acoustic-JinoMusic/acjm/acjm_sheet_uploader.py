...
def update_blacklist_from_sheet(sheet, blacklist_file):
    ...
    existing_pairs = {(item['link_ac'], item['link_jm']) for item in blacklist}
    new_blacklist_entries = []
    for row in all_data:
        if 'Feedback' in row and row['Feedback'] == 'Wrong':
            link_ac = row.get('Link_AC', '')
            link_jm = row.get('Link_JM', '')
            if (link_ac, link_jm) not in existing_pairs:
                entry = {
                    'link_ac': link_ac,
                    'link_jm': link_jm,
                ...
                existing_pairs.add((link_ac, link_jm))
...
def filter_data_against_blacklist(df, blacklist):
    ...
    blacklisted_pairs = {(item['link_ac'], item['link_jm']) for item in blacklist}
    filtered_df = df[~df.apply(
        lambda row: (row['Link_AC'], row['Link_JM']) in blacklisted_pairs,
        axis=1
    )]
...
def build_price_alert_message(store_changed, other_store, name_ac, name_other,
    ...
    if old_price is not None and new_price is not None:
        ...
        if diff < 0:
            head, arrow, icon = 'ფასი დაეცა', '🔻', '📉'
        else:
            head, arrow, icon = 'ფასი გაიზარდა', '🔺', '📈'
...
def detect_and_alert_price_changes(df_old, df_new, token, chat_id):
    ...
    cols = {
        ...
        'name_other': 'Product_Name_JM',
        'price_ac': 'Price_AC',
        'price_other': 'Price_JM',
        'link_ac': 'Link_AC',
        'link_other': 'Link_JM',
        'other_store': 'JinoMusic',
    }
    required_cols = [
...
def main():
    ...
    source_file = os.path.join(BASE_DIR, 'reports', 'acoustic_vs_jinomusic.xlsx')
