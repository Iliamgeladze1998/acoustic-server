...
export class LoginServer {
  ...
  private sessions: Map<string, string> = new Map(); // session -> userId
  private servers: GameServerInfo[] = [];
  private characters: Map<string, CharacterInfo[]> = new Map(); // userId -> chars

  constructor(port: number = 80, gameServer: GameServer, world: World) {
    this.port = port;
...
<update xmlns="http://ns.adobe.com/air/framework/update/description/2.5">
    ...
    this.app.post('/offline', (req, res) => {
      const act = req.body.act as string;
    ...
    this.app.post('/admin/gold', (req, res) => {
      const charId = req.body.character as string;
      const type = req.body.type as string || 'coppers';
    ...
    this.app.post('/admin/level', (req, res) => {
      const charId = req.body.character as string;
      const level = parseInt(req.body.level as string) || 1;
    ...
    this.app.post('/admin/stats', (req, res) => {
      const charId = req.body.character as string;
      const stats = req.body.stats as Record<string, number>;
    ...
    this.app.post('/admin/item', (req, res) => {
      ...
      const item = {
        id: 'item_' + Date.now(),
        slot: req.body.slot as string || 'inventory',
        type: req.body.type as string || 'weapon',
        bound: false,
        binding: 'none',
        templateId: req.body.templateId as string || 'sword_1',
        imagePath: req.body.imagePath as string || 'items/sword_1.png',
        itemClass: req.body.itemClass as string || 'weapon',
        count: parseInt(req.body.count as string) || 1,
        durability: 100,
        durabilityMaximum: 100,
        usable: false,
        sockets: 0,
        socketTypes: [],
        subType: req.body.subType as string || '',
        level: parseInt(req.body.level as string) || 1,
        items: [],
        autoAbilities: [],
        equipped: false,
        bodySlot: -1,
      ...
      if (this.world.addItem(charId, item as any)) {
  ...
  private async handleRegister(req: express.Request, res: express.Response): Promise<void> {
    ...
    const account: UserAccount = { id: userId, email: user, password };
  ...
  private handleLogin(req: express.Request, res: express.Response): void {
    const user = req.body.user as string;
    const password = req.body.password as string;
  ...
  private handleGetServers(req: express.Request, res: express.Response): void {
  ...
  private handleGetCharacters(req: express.Request, res: express.Response): void {
    const session = req.body.session as string;
  ...
  private handleGetToken(req: express.Request, res: express.Response): void {
    const session = req.body.session as string;
