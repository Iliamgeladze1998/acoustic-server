...
export class AMF3Reader {
  ...
  private readValue(marker: number): any {
    switch (marker) {
  ...
  private readAMF3Integer(): number {
    return this.readU29();
  }

  private readAMF3XmlDoc(): any {
    const ref = this.readU29();
    if ((ref & 1) === 0) {
  ...
  private readAMF3Object(): any {
    ...
    const obj: any = { __className: className };
  ...
  private readAMF3Xml(): any {
    return this.readAMF3XmlDoc();
  }

  private readAMF3ByteArray(): Buffer {
    const ref = this.readU29();
    if ((ref & 1) === 0) {
      return this.objects[ref >> 1];
    }
...
export class AMF3Writer {
  ...
  private strings: string[] = [];
  private stringRefs: Map<string, number> = new Map();
  private objects: any[] = [];
  private objectRefs: Map<any, number> = new Map();

  writeByte(val: number): void {
    this.buffers.push(Buffer.from([val & 0xFF]));
  }

  writeUnsignedByte(val: number): void {
    this.buffers.push(Buffer.from([val & 0xFF]));
  }
  ...
  writeUnsignedShort(val: number): void {
    const buf = Buffer.alloc(2);
  ...
  private writeU29(val: number): void {
    val = val >>> 0;
    if (val < 0x80) {
  ...
  private writeAMF3String(str: string): void {
    if (str === '') {
  ...
  private writeAMF3Array(arr: any[]): void {
  ...
  private writeAMF3Object(obj: any): void {
    ...
    if (className && sealedKeys.length > 0 && !isDynamic) {
      ...
      for (const key of Object.keys(obj)) {
        if (key === '__className' || key === '__sealed') continue;
        this.writeAMF3String(key);
        this.writeObject(obj[key]);
      }
