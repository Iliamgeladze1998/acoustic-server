   ...
   public class ListCollectionView extends Proxy implements ICollectionView, IList, IMXMLObject
   {
      ...
      private function getFilteredItemIndex(item:Object) : int
      {
         ...
         for(var i:int = loc - 1; i >= 0; i--)
         {
            ...
            if(Boolean(this.filterFunction(prevItem)))
            {
               len = int(this.localIndex.length);
               for(j = 0; j < len; j++)
               {
      ...
      private function addItemsToView(items:Array, sourceLocation:int, dispatch:Boolean = true) : int
      {
         ...
         if(Boolean(this.localIndex))
         {
            ...
            {
               item = items[i];
               if(this.filterFunction == null || Boolean(this.filterFunction(item)))
               {
                  ...
                  if(Boolean(this.sort) && Boolean(this.sort.unique) && this.sort.compareFunction(item,this.localIndex[loc]) == 0)
                  {
         ...
         if(Boolean(this.sort) || this.filterFunction != null)
         {
            ...
            if(this.filterFunction != null)
            {
               ...
               for(i = 0; i < len; i++)
               {
                  ...
                  if(Boolean(this.filterFunction(item)))
                  {
                     tmp.push(item);
                  }
               }
