   ...
   public class Sort extends EventDispatcher implements ISort
   {
      ...
      public function findItem(items:Array, values:Object, mode:String, returnInsertionIndex:Boolean = false, compareFunction:Function = null) : int
      {
         ...
         while(!objFound && lowerBound <= upperBound)
         {
            ...
            obj = items[index];
            direction = Boolean(fieldsForCompare) ? int(compareForFind(values,obj,fieldsForCompare)) : int(compareForFind(values,obj));
            switch(direction)
            {
               case -1:
                  upperBound = index - 1;
               ...
               case 0:
                  ...
                  switch(mode)
                  {
                     ...
                     case FIRST_INDEX_MODE:
                        ...
                        {
                           obj = items[objIndex];
                           prevCompare = Boolean(fieldsForCompare) ? int(compareForFind(values,obj,fieldsForCompare)) : int(compareForFind(values,obj));
                           match = prevCompare == 0;
                     ...
                     case LAST_INDEX_MODE:
                        ...
                        {
                           obj = items[objIndex];
                           nextCompare = Boolean(fieldsForCompare) ? int(compareForFind(values,obj,fieldsForCompare)) : int(compareForFind(values,obj));
                           match = nextCompare == 0;
