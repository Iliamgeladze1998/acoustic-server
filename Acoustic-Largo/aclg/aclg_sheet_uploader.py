...
PRICE_COLUMNS = ['Price_AC', 'Price_LG']
STORE_NAMES = {
    'Price_AC': 'Acoustic',
    'Price_LG': 'Largo',
}
...
def _ensure_recent_merged_file(merged_file):
    if not os.path.exists(merged_file):
        _abort_critical(f"Merged Excel file not found: {merged_file}. Run the merger before starting the uploader.")
    ...
    if age_seconds > MERGED_FILE_MAX_AGE_SECONDS:
        ...
        _abort_critical(f"Merged Excel file is older than 24 hours: {merged_file} (modified {modified_at}, age {age_hours:.2f} hours).")
    modified_at = datetime.fromtimestamp(file_mtime).strftime("%Y-%m-%d %H:%M:%S")
    age_hours = max(age_seconds, 0) / 3600
    print(f"✅ Freshness check passed: merged Excel modified at {modified_at} ({age_hours:.2f} hours old).")
            ...
            for idx, link in df[col].items():
                if pd.notna(link) and isinstance(link, str):
                    if 'AC' in col.upper():
                        continue
                    if 'LG' in col.upper():
                        if 'largo.ge' in link.lower():
                            parts = link.split('largo.ge/')
                            if len(parts) >= 2:
                                path = parts[1].lstrip('/')
                                if '%' not in path:
                                    encoded_path = urllib.parse.quote(path)
                                    final_url = f"https://largo.ge/{encoded_path}"
...
def update_blacklist_from_sheet(sheet, blacklist_file):
    ...
    existing_pairs = {(item['link_ac'], item['link_lg']) for item in blacklist}
    new_blacklist_entries = []
    for row in all_data:
        if 'Feedback' in row and row['Feedback'] == 'Wrong':
            link_ac = row.get('Link_AC', '')
            link_lg = row.get('Link_LG', '')
            if (link_ac, link_lg) not in existing_pairs:
                entry = {
                    'link_ac': link_ac,
                    'link_lg': link_lg,
                ...
                existing_pairs.add((link_ac, link_lg))
...
def filter_data_against_blacklist(df, blacklist):
    ...
    blacklisted_pairs = {(item['link_ac'], item['link_lg']) for item in blacklist}
    filtered_df = df[~df.apply(
        lambda row: (row['Link_AC'], row['Link_LG']) in blacklisted_pairs,
        axis=1
    )]
...
def apply_standard_sheet_formatting(worksheet, headers, row_count):
    ...
    for idx, header in enumerate(headers, start=1):
        ...
        if header in ['Price_AC', 'Price_LG', 'Price_Diff']:
...
def upload_to_sheet(df, sheet, sh):
    ...
    # Clear ALL existing data validation rules first
...
def detect_and_alert_price_changes(df_old, df_new, token, chat_id):
    ...
    cols = {
        ...
        'name_other': 'Product_Name_LG',
        'price_ac': 'Price_AC',
        'price_other': 'Price_LG',
        'link_ac': 'Link_AC',
        'link_other': 'Link_LG',
        'other_store': 'Largo',
    }
...
def main():
    ...
    source_file = os.path.join(BASE_DIR, 'reports', 'acoustic_vs_largo.xlsx')
